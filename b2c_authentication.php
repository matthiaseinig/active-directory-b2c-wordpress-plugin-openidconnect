	...
	try {
		if (isset($_POST['error'])) {
			...
			if (preg_match('/.*AADB2C90091.*/i', $_POST['error_description'])) {
				...
				wp_safe_redirect(B2C_Settings::$redirect_uri);
				exit;
			}
			...
			echo '<br><br><a href="' . B2C_Settings::$redirect_uri . '">Go to ' . B2C_Settings::$redirect_uri . '</a>';
			exit;
		}
		...
		if (isset($_POST[B2C_RESPONSE_MODE])) {	
			...
			if (in_array('administrator', $wp_user->roles)) {
				...
				// If there is a separate admin policy
				if(mb_strtolower(B2C_Settings::$admin_policy) != mb_strtolower(B2C_Settings::$generic_policy)){
